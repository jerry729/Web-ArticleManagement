...
%>
		...
		<main class="page-content">
			...
			<div class="wishlist-page-area section-padding-lg bg-white">
				<div class="container">
					...
					<%--<div><a href="addBbs.jsp" class="sf-button sf-button-dark">发布评论</a></div>--%>
					<!-- 下面写内容 -->
	<table class="layui-table table table-bordered table-hover" lay-filter="mylist" lay-size="lg">
		...
		<!-- 		<th lay-data="{field:'sj',align:'center', minWidth:100}">评论内容</th>
				<th lay-data="{field:'ly',align:'center', minWidth:100}">评论时间</th> -->
				<th lay-data="{field:'hn',align:'center', minWidth:100}">评论内容</th>
				<th lay-data="{field:'hf',align:'center', minWidth:100}">评论时间</th>
		...
		<tbody>
		 ...
		 <tr>
				<!-- <td></td> -->
				<td>${num.count }</td>
				<td><c:forEach items="${nlist }" var="b">
				<c:if test="${b.id==u.gid }">
				<a href="showNews.do?id=${b.id }">${b.name }</a>
				</c:if>
				</c:forEach>
				</td>
			<%-- 	<td>${u.note }</td>
				...
				<td>${u.pubtime }</td>
			  <td>
				<a class="layui-btn layui-btn-danger layui-btn-xs color2"  onclick="return del(1,1,1)" href="deleteBbsFore_pl.do?id=${u.id }" >删除</a>
			  </td>
			</tr>
        </c:forEach> 
		</tbody>
